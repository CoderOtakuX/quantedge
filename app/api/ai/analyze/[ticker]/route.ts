...
export async function GET(
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        // ── Phase 0: Fetch data from backend ──
        send("status", { step: 0, label: "Fetching market data..." });
        ...
        if (!overview) {
          send("error", { message: "Could not fetch stock overview" });
          controller.close();
          return;
        }
        ...
        send("status", { step: 0, label: "Market data loaded", done: true });

        // Prepare summaries
        const financialSummary = JSON.stringify({
          name: overview.name, sector: overview.sector,
          market_cap: overview.market_cap, pe_ratio: overview.pe_ratio,
          pb_ratio: overview.pb_ratio, roe: overview.roe,
          debt_to_equity: overview.debt_to_equity, net_margin: overview.net_margin,
          operating_margin: overview.operating_margin, revenue: overview.revenue,
          net_income: overview.net_income, eps: overview.eps,
          free_cash_flow: overview.free_cash_flow, dividend_yield: overview.dividend_yield,
          beta: overview.beta, current_ratio: overview.current_ratio,
        });

        const newsSummary = newsItems.length > 0
          ? newsItems.slice(0, 10).map((n: any, i: number) => `${i + 1}. ${n.title} (${n.source})`).join("\n")
          : "No recent news available.";

        const peerSummary = peers.length > 0
          ? JSON.stringify(peers.slice(0, 6).map((p: any) => ({
              symbol: p.symbol, pe_ratio: p.pe_ratio, pb_ratio: p.pb_ratio,
              roe: p.roe, market_cap: p.market_cap, net_margin: p.net_margin,
            })))
          : "No peer data available.";

        // ── Phase 1-3: Run steps in PARALLEL ──
        send("status", { step: 1, label: "Analysing financials..." });
        send("status", { step: 2, label: "Scanning news sentiment..." });
        send("status", { step: 3, label: "Benchmarking peers..." });
        ...
        const step1 = step1Result.status === "fulfilled" ? parseGroqJSON(step1Result.value) : null;
        const step2 = step2Result.status === "fulfilled" ? parseGroqJSON(step2Result.value) : null;
        const step3 = step3Result.status === "fulfilled" ? parseGroqJSON(step3Result.value) : null;

        send("step", { step: 1, label: "Financials complete", data: step1 });
        send("step", { step: 2, label: "Sentiment complete", data: step2 });
        send("step", { step: 3, label: "Peers complete", data: step3 });

        // ── Phase 4: Final synthesis (sequential — needs steps 1-3) ──
        send("status", { step: 4, label: "Synthesizing verdict..." });
      ...
      Return this exact JSON structure:
      {
        ...
        send("step", { step: 4, label: "Synthesis complete", data: step4 });
        ...
        console.error("AI analysis streaming error:", err);
        send("error", { message: err.message || "AI analysis failed" });
      } finally {
        controller.close();
      }
    },
...
async function runNonStreaming(ticker: string) {
  try {
    const [overview, newsRaw] = await Promise.all([
      safeFetch(`${BACKEND}/api/stock/${ticker}/overview`),
      safeFetch(`${BACKEND}/api/stock/${ticker}/news`),
    ...
    const [step1Result, step2Result, step3Result] = await Promise.allSettled([
      groqCall(`Analyze this company's financial data and return JSON only:\n${financialSummary}\n\nReturn this exact JSON structure:\n{\n  "strengths": ["string", "string", "string"],\n  "weaknesses": ["string", "string", "string"],\n  "growth_verdict": "string (one sentence)",\n  "financial_health_score": number between 0-100\n}`, STRICT_SYSTEM),
      groqCall(`Analyze these news headlines for ${ticker} and return JSON only:\n${newsSummary}\n\nReturn this exact JSON structure:\n{\n  "sentiment": "Bullish" | "Bearish" | "Neutral",\n  "sentiment_score": number between -100 to 100,\n  "catalysts": ["string", "string"],\n  "risks": ["string", "string"],\n  "news_summary": "string (one sentence)"\n}`, STRICT_SYSTEM),
      groqCall(`Compare ${ticker} against its sector peers using this data:\n\n${ticker} metrics: ${financialSummary}\n\nPeers: ${peerSummary}\n\nReturn this exact JSON structure:\n{\n  "relative_position": "Leader" | "Above Average" | "Average" | "Below Average" | "Laggard",\n  "pe_commentary": "string",\n  "roe_commentary": "string",\n  "margin_commentary": "string",\n  "valuation_verdict": "Overvalued" | "Fairly Valued" | "Undervalued"\n}`, STRICT_SYSTEM),
    ]);
